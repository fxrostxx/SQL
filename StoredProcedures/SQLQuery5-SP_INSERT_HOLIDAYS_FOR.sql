--SQLQuery5-SP_INSERT_HOLIDAYS_FOR.sql
...
BEGIN
	...
	CASE @holiday
		WHEN N'Нов%'            THEN dbo.GetNewYearHolidaysStartDay(@year)
		WHEN N'Пасха'           THEN dbo.GetEasterDate(@year)
		WHEN N'Летние каникулы' THEN dbo.GetSummerHolidaysStartDay(@year)
		                        ELSE DATEFROMPARTS(@year, @month, @day)
	END
	--IF @month IS NOT NULL AND @day IS NOT NULL SET @start_date = DATEFROMPARTS(@year, @month, @day);
	--IF @holiday LIKE N'Нов%'                   SET @start_date = dbo.GetNewYearHolidaysStartDay(@year);
	--IF @holiday LIKE N'Пасха'                  SET @start_date = dbo.GetEasterDate(@year);
	--IF @holiday LIKE N'Летние каникулы'        SET @start_date = dbo.GetSummerHolidaysStartDay(@year);
